...
The server above exposes a `GET /` endpoint that accepts a `Authorization` header with a Kliento token bundle, and requires the `audience` to be set to the URL of the request.
...
export EXCHANGE_JWT=$(curl --request POST \
  ...
  --header 'Content-Type: application/x-www-form-urlencoded' \
  --data-urlencode grant_type=client_credentials \
  --data-urlencode client_id=machine@cloud-provider.example \
  --data-urlencode client_secret=s3cr3t \
...
curl --request GET \
  ...
  --header "Authorization: Bearer $TOKEN_BUNDLE"
```
...
Congratulations! You've successfully set up a complete Kliento system on your local machine. Let's recap what we've accomplished:

1. We set up a local VeraId Authority instance to manage your domain's credentials.
2. We implemented a simple server that verifies Kliento token bundles.
3. We configured your domain in VeraId Authority.
4. We created a user or bot identity associated with your domain.
5. We configured a signature spec that allows the exchange of JWTs for Kliento token bundles.
6. We demonstrated the complete flow: obtaining a JWT, exchanging it for a token bundle, and using that bundle to make an authenticated request.

This hands-on exercise has shown you the fundamental components of Kliento authentication and how they work together to provide domain-based identity for your applications.
...
- [Learn about integrating Kliento in your clients](/kliento/clients).
- [Learn about integrating Kliento in your servers](/kliento/servers).
